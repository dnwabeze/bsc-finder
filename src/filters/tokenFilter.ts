...
export function passesFilters(token: TokenInfo): boolean {
  ...
  // ── Name / ticker match ───────────────────────────────────────────────────
  // Strip $ from both the token and the keyword before comparing
  // so "STC" matches "$STC" and vice versa
  ...
  if (twitterMatches || telegramMatches || websiteMatches) return true;
